/* 
Bernard Williams
UCF Bootcamp APR2017 Week 4
RPG Starwars Game
*/

//Characters array holding all of the game characters as objects
var characters = [
  {
    name: "Darth Vader",
    imgURL: "assets/images/darthVader.jpg",
    desc: "This Captain of Chaos with a bad respiratory issue will never leave you wanting for less destruction.  Luke... I am your father!",
    id: "dv",
    attackPower: 0,
    healthPoints: 0,
    counterAttackPower: 0
  },  
  {
    name: "Luke Skywalker",
    imgURL: "assets/images/luke.png",
    desc: "Yes.  He has Daddy issues.  But can you really blame him.  His Dad has made a lot of bad choices.",
    id: "ls",
    attackPower: 0,
    healthPoints: 0,
    counterAttackPower: 0
  },
  {
    name: "Hans Solo",
    imgURL: "assets/images/hans-solo.jpg",
    desc: "Suave... Debonair... and he got to make out with Carrie Fisher!!  Chewy was the heavy though.",
    id: "hs",
    attackPower: 0,
    healthPoints: 0,
    counterAttackPower: 0
  },
  {
    name: "Kylo-Ren",
    imgURL: "assets/images/kylo-ren.jpg",
    desc: "Angst, angst, angst!!!  This son of a princess needs a week with Serina Williams to set her straight",
    id: "kr",
    attackPower: 0,
    healthPoints: 0,
    counterAttackPower: 0
  },
  {
    name: "R2D2",
    imgURL: "assets/images/r2d2.jpg",
    desc: "Part machine, part asshole.  This trashcan look alike has a lot of attitude and ALWAYS saves the day.",
    id: "rs",
    attackPower: 0,
    healthPoints: 0,
    counterAttackPower: 0
  }
];

